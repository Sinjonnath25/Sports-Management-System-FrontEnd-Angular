...
export class TrainerSignupComponent implements OnInit {
  ...
  formSubmit() {
    ...
    if (this.user.password.length <6) {
      // alert('User is required !!');
      this.snack.open('Password should have minimum 6 characters !! ', '', {
        duration: 3000,
      });
      return;
    }
    ...
    if (this.user.phone.length < 10 ||this.user.phone.length > 10 || this.user.phone == null ||this.user.phone =='') {
      // alert('User is required !!');
    ...
    //addUser: userservice
    this.userService.addTrainer(this.user).subscribe(
      (data: any) => {
        //success
        console.log(data);
        //alert('success');
